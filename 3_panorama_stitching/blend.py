...
def imageBoundingBox(img, M):
    ...
    # transformed_corners = cv2.perspectiveTransform(np.array([corners], dtype=np.float32), M)
    ...
    # print('transformed corners {}'.format(transformed_corners))
    normalized_corners = transformed_corners[:, :2] / transformed_corners[:, 2][:, None]
    # print('normalized corners {}'.format(normalized_corners))
...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    for i in range(minX, maxX):
        for j in range(minY, maxY):
            cur_point = np.dot(M_inv, np.array([[i, j, 1]]).T)
            x, y = int(cur_point[0][0] / cur_point[2][0]), int(cur_point[1][0] / cur_point[2][0])

            if 0 <= x < width - 1 and 0 <= y < height - 1 and not np.all(img[y, x] == 0):
                c1 = ((i - minX) / blendWidth) if (i < minX + blendWidth) else 1.0
                c2 = ((maxX - i) / blendWidth) if (i > maxX - blendWidth) else 1.0
                weight = min(c1, c2, 1.0)
                RGB = img[y, x, :3]
                acc[j, i, :3] += RGB * weight   
                acc[j, i, 3] += weight
    # #TODO-BLOCK-END
    # END TODO
...
def getAccSize(ipv):
    ...
    for i in ipv:
        ...
        #TODO-BLOCK-BEGIN
        cur_img_min_x, cur_img_min_y, cur_img_max_x, cur_img_max_y = imageBoundingBox(img,M)
        minX = min(cur_img_min_x, minX)
        minY = min(cur_img_min_y, minY)
        maxX = max(cur_img_max_x, maxX)
        maxY = max(cur_img_max_y, maxY)        
        #TODO-BLOCK-END
        # END TODO
