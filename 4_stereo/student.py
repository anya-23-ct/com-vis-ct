...
def compute_photometric_stereo_impl(lights, images):
    ...
    images_reshaped = np.reshape(images, (N, -1))
    G = np.dot(np.linalg.inv(np.dot(lights.T, lights)), np.dot(lights.T, images_reshaped))
    c = np.reshape(G.T,(height,width,channels,3))
    albedo = np.linalg.norm(c, axis = 3)
    gray = np.mean(c, axis=2)
    tmp = np.maximum(np.linalg.norm(gray, axis=2, keepdims=True), 1e-7)
    normals = gray / tmp
...
def project_impl(K, Rt, points):
    ...
    for i in range(len(points)):
        for j in range(len(points[i])):
            c = np.array(points[i, j])
            h = p.dot(np.array([c[0], c[1], c[2], 1.0]))
            proj[i, j] = np.array([h[0] / h[2], h[1] / h[2]])
    return proj
...
def preprocess_ncc_impl(image, ncc_size):
    ...
    for i in range(half_patch, height-half_patch):
        for j in range(half_patch, width-half_patch):
            normalized[i][j] = np.copy(image[i-half_patch:i+half_patch+1 , j-half_patch:j+half_patch+1 , :])
    
    normalized = normalized.reshape((height, width, ncc_size**2, channels))
    normalized = normalized.transpose((0, 1, 3, 2))
    normalized -= np.mean(normalized, axis = 3, keepdims = True)

    normalized_norms = np.linalg.norm(normalized,axis = (2,3), keepdims = True)
    normalized /= np.maximum(1e-6,normalized_norms)
